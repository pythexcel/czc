...
function SideNav() {
  ...
  const [selectedItem, setSelectedItem] = useState("bots");
  ...
  return (
    <div
      className={`flex flex-col h-[100vh] shadow-xl ${
        open ? "w-[230px]" : "w-20"
      } duration-300`}
    >
      <div className="bg-slate-50 flex flex-row h-[60px] justify-between items-center">
        <div className="flex ml-5 gap-2 h-[60px] cursor-pointer">
          ...
          <span className="text-[#5BC2FE] font-bold my-auto text-xl">
            {open && "ZappyChat"}
          </span>
        </div>
        <div className="flex-col items-start cursor-pointer">
          <CgMenuRight
            onClick={() => setOpen(!open)}
            className="text-gray-500 my-auto h-[25px] w-[30px]"
          />
        </div>
      </div>
        ...
        <ul className="relative m-0 list-none items-center justify-center text-center">
          {Navi.map((item, i) => (
            <li
              ...
              className={`cursor-pointer relative flex items-center mx-auto gap-2 my-1 ${
            ...
            >
              ...
              <span
                className={`font-medium ${!open && !hoverOpen && "hidden"}`}
              >
                {item.name}
              </span>
            </li>
