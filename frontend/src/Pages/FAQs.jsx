...
const FAQs = () => {
  ...
  const handleSearchUniquefaqs = (event) => {
    ...
    setSearchItem(newOne)
    if (newOne) {
      const filterfq = maindata.filter((item) => item.location_name.toLowerCase().includes(newOne.toLowerCase())
      );
      setData(filterfq)
    }
    else {
  ...
  const handleCallOptions = async () => {
    ...
    try {
      ...
      setOpt(response?.details.map(item => item.bot_name));
    } catch (error) {
